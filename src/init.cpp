...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in EXOTIC/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in EXOTIC/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in EXOTIC/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all EXOTIC specific functionality (Masternodes, Darksend, InstantX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-anonymizeexoticamount=<n>", strprintf(_("Keep N EXOTIC anonymized (default: %u)"), 0));
...
/** Sanity checks
 *  Ensure that EXOTIC is running in a usable environment with all
...
#endif
    // Make sure only a single EXOTIC process is using the data directory.
...
// ********************************************************* Step 8: load wallet
    ...
    nAnonymizeEXOTICAmount = GetArg("-anonymizeexoticamount", 0);
    if (nAnonymizeEXOTICAmount > 999999) nAnonymizeEXOTICAmount = 999999;
    if (nAnonymizeEXOTICAmount < 2) nAnonymizeEXOTICAmount = 2;
    ...
    LogPrintf("Anonymize EXOTIC Amount %d\n", nAnonymizeEXOTICAmount);
    ...
    /* Denominations
       ...
       For example:
       1EXOTIC+1000 == (.1EXOTIC+100)*10
       10EXOTIC+10000 == (1EXOTIC+1000)*10
    */
